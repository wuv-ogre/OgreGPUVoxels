...
vec3 GetUVW(vec3 coord)
{
    return (2.0 * (coord * g_step_pixels + g_margin_pixels + g_quadrant_offset_pixels) + 1.0) * (0.5 * g_inv_density_volume_size.x);
}

ivec3 GetTexel(vec3 coord)
{
    return ivec3( coord * g_step_pixels + g_margin_pixels + g_quadrant_offset_pixels );
}
...
const ivec3 CubeFaces[6] = ivec3[6]
(
...
void main()
{
    ...
    if (cube_case * (255u-cube_case) > 0u)
    {
        for(int i=0; i<6; i++)
        {
            uint other = CubeCase(texel + CubeFaces[i]);
        }
    }
    else
    {
